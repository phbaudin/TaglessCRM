...
This DAG will transfer data from BigQuery to Google Ads Store Sales Conversions.
...
class BigQueryToAdsSSDDag(base_dag.BaseDag):
  """BigQuery to Google Ads Store Sales Conversion DAG."""
  ...
  def create_task(
    ...
    return data_connector_operator.DataConnectorOperator(
        dag_name=_DAG_NAME,
        task_id=self.get_task_id('bq_to_ads_ssd', is_retry),
...
if os.getenv(_AIRFLOW_ENV):
  dag = BigQueryToAdsSSDDag(_DAG_NAME).create_dag()
